    ...
    public class port_moudbus
    {
        ...
        private void sp_DataReceived(object sender,SerialDataReceivedEventArgs e)
        {
            try
            {
                ...
                //sp.Read(array_char, 0, readcount);
                if (readcount >= 30)
                {
                    readcount = 229;
                }
                ...
                if (send_machine_num == array[0] && send_gongnengma == array[1] && send_is == true && receive_is == false)
                {
                    ...
                    Console.WriteLine("数量为:"+readcount.ToString());
                    if (array[1] == 3)
                    {
                        ...
                        //Console.WriteLine(RealTime_data.DT[array[0],44].ToString());
                    }
                    //解析R的值并根据R值在面页上做出变化
                    ...
                    //Console.WriteLine(RealTime_data.DT[array[0], 62].ToString());
        ...
        public void Send_Write_Cmd16(int machine_num, int datalength, string address, int[] Date)
        {
            ...
            string cmd_string = "";
            SerialPort sp = new SerialPort();
            int address2 = int.Parse(address,System.Globalization.NumberStyles.HexNumber);
            address2 = address2 + 80;
            string address2_string = address2.ToString("X").PadLeft(4, '0');
            string data_string = "";
            if (Date.Length <= 80)
            {
            ...
            else
            {
                ...
                for (int i = 0; i < 80; i++)
                {
                    data_string1 = data_string1 + (Date[i] / 256).ToString("X").PadLeft(2, '0') + (Date[i] % 256).ToString("X").PadLeft(2, '0');
                }

                cmd_string="";
                cmd_string = machine_num.ToString("X").PadLeft(2, '0') + "10" + address + (80 / 256).ToString("X").PadLeft(2, '0') + (80 % 256).ToString("X").PadLeft(2, '0') + (2 * 80).ToString("X").PadLeft(2, '0') + data_string1;
                xunjian_is = false;
                Thread.Sleep(500);
                send_data(cmd_string);

                for (int i = 80; i < Date.Length; i++)
                {
                    data_string2 = data_string2 + (Date[i] / 256).ToString("X").PadLeft(2, '0') + (Date[i] % 256).ToString("X").PadLeft(2, '0');
                }
                cmd_string = machine_num.ToString("X").PadLeft(2, '0') + "10" + address2_string + ((datalength - 80) / 256).ToString("X").PadLeft(2, '0') + ((datalength - 80) % 256).ToString("X").PadLeft(2, '0') + (2 * (datalength - 80)).ToString("X").PadLeft(2, '0') + data_string2;
                Thread.Sleep(500);
                send_data(cmd_string);
            }
            try 
